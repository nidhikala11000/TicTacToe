...
/**
 * used to create board of [3][3] marix
 */
...
public class Board {
	...
	// to check if anyone won or not
	public boolean hasPlayerWon(int player) {
		if ((board[0][0] == board[1][1] && board[0][0] == board[2][2] && board[0][0] == player) // used to check
				// diagonally
		...
		for (int i = 0; i < 3; i++) {
			if ((board[i][0] == board[i][1] && board[i][0] == board[i][2] && board[i][0] == player) // used to check row
					// and column wise
	...
	// to put a specific value in cell
	...
	public int minimax(int depth, int turn) {
		...
		for (int i = 0; i < availableCells.size(); i++) {
			...
			if (turn == PLAYER_X) {
				...
				if (depth == 0) {
					// System.out.println("Computer Score For position " + point + " - " +
					// currentScore);
				}
